...
function unlockAudio(audio){audio.volume=0;audio.play();audio.pause();audio.currentTime=0;audio.volume=1;}
function unlockAudios(){unlockAudio(correctAllAudio);unlockAudio(correctAudio);unlockAudio(incorrectAudio);unlockAudio(stupidAudio);}
function setScoringButton(problemBox,tegakiPanel,tehonPanel,objects,tegakiPads,word){var scoring=problemBox.shadowRoot.querySelector('#scoring');scoring.addEventListener('click',function(){getProblemScores(tegakiPanel,tehonPanel,objects,tegakiPads).then(scores=>{if(scores.every(score=>score>=80)){problemBox.shadowRoot.querySelector('#guard').style.height='100%';var next=problemBox.nextElementSibling;if(next){next.shadowRoot.querySelector('#guard').style.height='0';const headerHeight=document.getElementById('header').offsetHeight;const top=next.getBoundingClientRect().top+document.documentElement.scrollTop-headerHeight;window.scrollTo({top:top,behavior:'smooth'});}else{window.removeEventListener('touchstart',scrollEvent,{passive:false});window.removeEventListener('touchmove',scrollEvent,{passive:false});}}
...
var kakuScore=lineScore*inclusionScore*100*2.8;if(kakuScore<0){kakuScore=0;}
...
score/=scores.length;if(score>=80){correctAllAudio.play();var clearedKanjis=localStorage.getItem('touch-50on');if(clearedKanjis){kanjis.split('').forEach(kanji=>{if(!clearedKanjis.includes(kanji)){clearedKanjis+=kanji;}});localStorage.setItem('touch-50on',clearedKanjis);}else{localStorage.setItem('touch-50on',kanjis);}
document.getElementById('report').classList.add('d-none');document.getElementById('correctReport').classList.remove('d-none');setTimeout(()=>{location.href='/touch-50on/';},3000);}else{stupidAudio.play();document.getElementById('report').classList.add('d-none');document.getElementById('incorrectReport').classList.remove('d-none');setTimeout(function(){document.getElementById('report').classList.remove('d-none');document.getElementById('incorrectReport').classList.add('d-none');},6000);}}
...
return str;}
let kanjis='';let mode='hirahira';function initQuery(){var problems1,problems2;var queries=parseQuery(location.search);mode=queries['mode'];kanjis=queries['q'];var problemQuery=queries['problem'];if(kanjis){if(mode=='conv'){var conved=convHirakana(kanjis);problems1=kanjis.split('');problems2=conved.split('');}else{problems1=kanjis.split('');problems2=kanjis.split('');}}else{if(problemQuery=='50on'){var hira50on=Array.from('あいうえおかきくけこさしすせそたちつてとなにぬねのはひふへほまみむめもやゆよわん');var kana50on=Array.from('アイウエオカキクケコサシスセソタチツテトナニヌネノハヒフヘホマミムメモヤユヨワン');if(mode=='hirahira'){problems1=hira50on;problems2=hira50on;}else if(mode=='hirakana'){problems1=hira50on;problems2=kana50on;}else if(mode=='kanakana'){problems1=kana50on;problems2=kana50on;}else{problems1=kana50on;problems2=hira50on;}}else{var hiradakuon=Array.from('がぎぐげござじずぜぞだぢづでどばびぶべぼぱぴぷぺぽ');var kanadakuon=Array.from('ガギグゲゴザジズゼゾダヂヅデドバビブベボパピプペポ');if(mode=='hirahira'){problems1=hiradakuon;problems2=hiradakuon;}else if(mode=='hirakana'){problems1=hiradakuon;problems2=kanadakuon;}else if(mode=='kanakana'){problems1=kanadakuon;problems2=kanadakuon;}else{problems1=kanadakuon;problems2=hiradakuon;}}}
loadDrill(problems1,problems2);document.getElementById('problems').children[0].shadowRoot.querySelector('#guard').style.height='0';}
function scrollEvent(e){if(!['MAIN','PROBLEM-BOX','A','BUTTON'].includes(e.target.tagName)){e.preventDefault();}}
window.addEventListener("touchstart",scrollEvent,{passive:false});window.addEventListener("touchmove",scrollEvent,{passive:false});document.addEventListener("touchstart",unlockAudios,{once:true});